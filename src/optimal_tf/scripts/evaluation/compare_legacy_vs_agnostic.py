...
from optimal_tf.allocation import compute_strategy_panel, supported_strategies
...
from trading_core.risk import estimate_clean_correlation_panel, estimate_clean_covariance_panel

DEFAULT_LEGACY = ("ARP", "NM")
DEFAULT_AGNOSTIC = ("ARP_AGNOSTIC", "MARKOWITZ_AGNOSTIC", "PHI_50", "ATF_AGNOSTIC")
DEFAULT_OUTPUT_DIR = "output/optimal_tf/evaluation/legacy_vs_agnostic"


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Compare legacy optimal_tf strategies against agnostic Eq. 8 recipes.")
    ...
    parser.add_argument("--legacy-strategies", type=str, default=",".join(DEFAULT_LEGACY))
    parser.add_argument("--agnostic-recipes", type=str, default=",".join(DEFAULT_AGNOSTIC))
...
def _validate_legacy(strategies: list[str]) -> None:
    allowed = supported_strategies()
    invalid = [strategy for strategy in strategies if strategy not in allowed]
    if invalid:
        raise ValueError(f"Unknown legacy strategies {invalid}. Allowed values: {allowed}")


def _validate_agnostic(recipes: list[str]) -> None:
...
def run(argv: list[str] | None = None) -> int:
    ...
    legacy = parse_csv_list(args.legacy_strategies)
    agnostic = parse_csv_list(args.agnostic_recipes)
    _validate_legacy(legacy)
    _validate_agnostic(agnostic)
    ...
    all_names = [*legacy, *agnostic]
    agnostic_set = set(agnostic)

    def _evaluate_dispatch(prices_frame, estimation_cfg, backtest_cfg, evaluation_cfg):
        strategy_name = str(evaluation_cfg.strategy)
        if strategy_name in agnostic_set:
    ...
    result = compare_strategies(
        ...
        evaluation,
        all_names,
        evaluate_portfolio_fn=_evaluate_dispatch,
    )

    print(f"universe: {universe.name}")
    print(f"legacy_strategies: {', '.join(legacy)}")
    print(f"agnostic_recipes: {', '.join(agnostic)}")
    ...
    if outdir is not None:
        ...
        if args.output_plot:
            render_series_comparison_plot(
                ...
                title="Legacy vs agnostic comparison",
                ylabel="NAV",
            )
