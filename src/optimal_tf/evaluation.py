...
from .allocation import compute_weights_panel
...
def _apply_portfolio_vol_target(
    ...
    # Shift the scale by one day so the targeter uses only information known at
...
def evaluate_portfolio(
    ...
    full_weights = compute_weights_panel(prices, est_cfg, eval_cfg.strategy, long_only=bt_cfg.long_only)
    weights_by_rebalance = full_weights.loc[rebalance_dates].copy()
    ...
    for pos, rebalance_date in enumerate(rebalance_dates):
        current_weights = weights_by_rebalance.loc[rebalance_date].fillna(0.0)
    ...
    daily_gross, _ = _apply_portfolio_vol_target(daily_gross, bt_cfg)
