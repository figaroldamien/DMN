...
from trading_core.reporting.metrics import evaluation_metrics
...
def apply_portfolio_vol_target(
    ...
    # Vol targeting is applied after the portfolio has been aggregated across
    # assets. It therefore acts as a portfolio-level overlay, not as a change
    # in the cross-sectional allocation itself.
...
def slice_next_holding_period(
    index: pd.DatetimeIndex,
    current: pd.Timestamp,
    next_rebalance: pd.Timestamp | None,
    evaluation_end: pd.Timestamp | None,
) -> pd.DatetimeIndex:
    # A rebalance decided on date t is applied strictly after t, up to the next
    # rebalance date included. This convention keeps signal observation and
    # realized holding-period returns separated by one step.
    mask = index > current
...
def evaluate_portfolio(
    ...
    eval_cfg: Any,
    *,
    compute_strategy_panel_fn: Callable[..., object],
    estimate_clean_covariance_panel_fn: Callable[..., dict[pd.Timestamp, pd.DataFrame]],
) -> EvaluationResult:
    # The evaluation engine works in three stages:
    # 1. resolve rebalance dates and compute target strategy states on those dates
    # 2. roll each rebalance forward across its holding period to build daily PnL
    # 3. apply portfolio-level overlays and summarize the resulting return stream
    ...
    # dates, which keeps the iterative backtest fast even for LLTF.
    ...
    # Iterate rebalance by rebalance. Each target weight vector is held until
    ...
    # Costs are applied on the first realized day after the rebalance, whereas
    # vol targeting rescales the whole daily gross-return series afterwards.
    daily_gross, portfolio_vol_scale = apply_portfolio_vol_target(daily_gross, bt_cfg)
    ...
    # Holding-period returns are exported separately from daily returns because
    # they are useful for diagnostics at the rebalance frequency itself.
