...
def render_evaluation_plot(
    ...
    *,
    title: str,
    benchmark_label: str = "universe equal-weight index",
    buy_hold_label: str = "equal-weight buy and hold",
) -> Path:
    ...
    ax.plot(benchmark_nav.index, benchmark_nav.values, label=benchmark_label, linewidth=1.8)
    ax.plot(buy_hold_nav.index, buy_hold_nav.values, label=buy_hold_label, linewidth=1.8)
    ax.set_title(title)
